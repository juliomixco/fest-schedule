...
function ScheduleRow({ act, stageColor, stageName, conflicted, festivalId }: ScheduleRowProps) {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: `${stageColor}22`, borderLeft: `4px solid ${stageColor}` }}
      onClick={() => toggleAct(festivalId, act.id)}
    >
      <img
        src={thumbnail ?? 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><circle cx="32" cy="24" r="12" fill="%23555"/><ellipse cx="32" cy="50" rx="20" ry="14" fill="%23555"/></svg>'}
        alt={act.name}
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm text-neutral-300">{start} – {end}</p>
      </div>
      {conflicted && (
        <span className="text-yellow-400 text-lg" title="Overlaps with another selected act">⚠</span>
      )}
    </div>
  );
}
...
export function MyScheduleView({ festival, festivalId }: MyScheduleViewProps) {
      ...
      day.stages.forEach((stage) => {
        stage.acts.forEach((act) => {
          if (isSelected(festivalId, act.id)) {
            acts.push({ ...act, stageColor: stage.color ?? '#6b7280', stageName: stage.name });
          }
        });
      });
    });
    return acts.sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime());
  ...
  if (selectedActs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-neutral-500">
        ...
        <p className="text-sm mt-1">Go to the Timeline and tap acts to add them here.</p>
      </div>
    );
  }
  ...
  selectedActs.forEach((act, i) => {
    if (i > 0) {
      ...
      const gap = differenceInMinutes(new Date(act.startTime), new Date(prev.endTime));
